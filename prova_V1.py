...
def pid_try(scf):
	with MotionCommander(scf, default_height=DEFAULT_HEIGHT) as mc:
		...
		while(1):
			...
			pdes= np.array([R * math.cos(w * t), R* math.sin(w * t), 0.05*t+0.5]).reshape(3,1)
...
def log_thrust_callback(timestamp, data, logconf):
	global thrust_estimate 
	thrust_estimate =data['stateEstimate.thrust']
