...
def pid_try(scf):
	with MotionCommander(scf, default_height=DEFAULT_HEIGHT) as mc:
		...
		while(1):
			...
			pdes , pddes , pdddes = trajectory_function('horizontal_circle',t)
			...
			if h_dot > 0 or h > delta:
				...
				print("ostacolo")
				a_diverso = - 1/mu * np.linalg.pinv(M) @ Proj @ (2*R_T @ M @ R @ V_dot + (R_dot_T @ M @ R + R_T @ M @ R_dot) @ V) + a_bar + Proj_perp @ a 
				
				A_MAX = 2.0 
				#saturo 
				a_diverso = np.clip(a_diverso, -A_MAX, A_MAX)
