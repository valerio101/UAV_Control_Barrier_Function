...
X_drone = squeeze(drone_x );
Y_drone = squeeze(drone_y );
Z_drone = squeeze( drone_z);
X_drone2 = squeeze(prova);
Y_drone2 = squeeze(19);
Z_drone2 = squeeze( 0);
% X_drone3 = squeeze( out.position.data(7,:));
% Y_drone3 = squeeze( out.position.data(8,:));
% Z_drone3 = squeeze( out.position.data(9,:));
% X_load = squeeze( out.position.data(10,:));
% Y_load = squeeze( out.position.data(11,:));
% Z_load = squeeze( out.position.data(12,:));
...
[x_real2, y_real2, z_real2] = generateSphere([X_drone2(1),0.5, 0], 0.5);

% [x_real3, y_real3, z_real3] = generateEllipsoid([X_drone3(1), Y_drone3(1), Z_drone3(1)], 0.5,0.5,0.25);
% 
% [x_ref, y_ref, z_ref] = generateSphere([X_load(1), Y_load(1), Z_load(1)], 0.5);
...
% surface_real3 = surf(x_real3, y_real3, z_real3, ...
%     'FaceColor', [1, 0.9216, 1.0], 'EdgeColor', 'none', 'FaceAlpha', 0.5);
% 
% surface_load = surf(x_ref, y_ref, z_ref, ...
...
trail_real2 = line('XData', [], 'YData', [], 'ZData', [], 'Color', [0.2196, 0.9216, 1.0], 'LineWidth', 3);
% trail_real3 = line('XData', [], 'YData', [], 'ZData', [], 'Color', [1, 0.9216, 1.0], 'LineWidth', 3);
% trail_load = line('XData', [], 'YData', [], 'ZData', [], 'Color', [0 0.5 1], 'LineWidth', 3);
...
% plot3(X_drone3,Y_drone3,Z_drone3,'g--', 'LineWidth', 2)
% plot3(X_load,Y_load,Z_load,'y--', 'LineWidth', 2)
h = hgtransform;
...
for k = 2:length(X_drone)
    ...
    [x_real, y_real, z_real] = generateEllipsoid([X_drone(k), Y_drone(k), Z_drone(k)], 0.5,0.5,0.25);
    set(surface_real, 'XData', x_real, 'YData', y_real, 'ZData', z_real);
    [x_real2, y_real2, z_real2] = generateSphere([X_drone2(k), 0.5, 0], 0.5);
    set(surface_real2, 'XData', x_real2, 'YData', y_real2, 'ZData', z_real2);
    % [x_real3, y_real3, z_real3] = generateEllipsoid([X_drone3(k), Y_drone3(k), Z_drone3(k)], 0.5,0.5,0.25);
    % set(surface_real3, 'XData', x_real3, 'YData', y_real3, 'ZData', z_real3);
    % [x_ref, y_ref, z_ref] = generateSphere([X_load(k), Y_load(k), Z_load(k)], 0.5);
    % set(surface_load, 'XData', x_ref, 'YData', y_ref, 'ZData', z_ref);
    % Aggiornamento posizione robot di riferimento
    rotoTranslateEllipsoid(surface_real,0.5,0.5,0.25, [X_drone(k), Y_drone(k), Z_drone(k)], eye(3));
    %rotoTranslateEllipsoid(surface_real2,0.5,0.5,0.25, [X_drone2(k), Y_drone2(k), Z_drone2(k)], eye(3));
    % rotoTranslateEllipsoid(surface_real3,0.5,0.5,0.25, [X_drone3(k), Y_drone3(k), Z_drone3(k)], eye(3));
    ...
    x_trail_real2 = [get(trail_real2, 'XData'), X_drone2(k)];
    y_trail_real2 = [get(trail_real2, 'YData'), 0.5];
    z_trail_real2 = [get(trail_real2, 'ZData'), 0];
    set(trail_real2, 'XData', x_trail_real2, 'YData', y_trail_real2, 'ZData', z_trail_real2);

    % x_trail_real3 = [get(trail_real3, 'XData'), X_drone3(k)];
    % y_trail_real3 = [get(trail_real3, 'YData'), Y_drone3(k)];
    % z_trail_real3 = [get(trail_real3, 'ZData'), Z_drone3(k)];
    % set(trail_real3, 'XData', x_trail_real3, 'YData', y_trail_real3, 'ZData', z_trail_real3);
    % 
    % x_trail_ref = [get(trail_load, 'XData'), X_load(k)];
    % y_trail_ref = [get(trail_load, 'YData'), Y_load(k)];
    % z_trail_ref = [get(trail_load, 'ZData'), Z_load(k)];
    % set(trail_load, 'XData', x_trail_ref, 'YData', y_trail_ref, 'ZData', z_trail_ref);
