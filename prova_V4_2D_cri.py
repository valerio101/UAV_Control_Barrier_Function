...
def pid_try(scf):
    with MotionCommander(scf, default_height=DEFAULT_HEIGHT) as mc:
        ...
        range_cov = [range(0)/1000,range(1)/1000,range(2)/1000,range(3)/1000]
        #posizione ostacolo 
        """
        obstacles_positions = [ 
        ...
        while(1):
            ...
            pdes , pddes , pdddes = trajectory_function('linear motion',t)
            ...
            range_cov = [np.array([range[0]/1000]),
                         np.array([range[1]/1000]),
                         np.array([range[2]/1000]),
                         np.array([range[3]/1000])]
            ...
            min_dist = float('inf')
            closest_idx = -1
            for i, obs_pos in enumerate(range_cov):
                dist = obs_pos
                if dist < min_dist:
                    min_dist = dist
                    closest_idx = i
            ...
            # Ostacolo scelto
            if(closest_idx==0):
                V= np.array([range_cov[closest_idx],0]).reshape(2,1)
            elif(closest_idx==1):
                V = np.array([-range_cov[closest_idx],0]).reshape(2,1)
            elif(closest_idx==2):
                V= np.array([0,+range_cov[closest_idx]]).reshape(2,1)
            else:
                V= np.array([0,-range_cov[closest_idx]]).reshape(2,1)
            #p_bar = range_cov[closest_idx]
            v_bar = [0,0].reshape(2,1)
            a_bar = [0,0].reshape(2,1)
            ...
            if h_dot > 0 or h > delta:
                ...
                a_diverso = - 1/mu *  Proj @ (2* V_dot) + Proj_perp @ a 
